...
BouncingSnowball::active_update(float dt_sec)
{
  ...
  bool groundBelow = !Sector::get().is_free_of_tiles(lookbelow) && !Sector::get().is_free_of_objects(lookbelow, COLGROUP_STATIC);
  ...
  if (!Sector::get().is_free_of_tiles(side_look_box) && !Sector::get().is_free_of_statics(side_look_box))
  {
